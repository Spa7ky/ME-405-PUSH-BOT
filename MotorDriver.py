...
class MotorDriver:
    ...
    def __init__(self,enablePin1='PB10',motorPin1='PB4',motorPin2='PB5',motortimer=3,level_limit=100):
        ...
        self.motorCh1 = self.motortimer.channel (1,pyb.Timer.PWM,pin = self.motorPin1,pulse_width_percent=0)
        ## Creates Ch2 for the Timer that has been input.
        self.motorCh2 = self.motortimer.channel (2,pyb.Timer.PWM,pin = self.motorPin2,pulse_width_percent=0)
    ...
    def set_duty_cycle(self,level=0):
        ...
        elif level < (-1)*(self.level_limit):
            level = (-1)*(self.level_limit)
        if level < 0:
